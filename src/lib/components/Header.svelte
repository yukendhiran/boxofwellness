...
<header class="p-4 md:p-6">
	...
	>
		<div class="flex items-center justify-between">
			<a href="/" on:click={close} class="md: z-50 flex items-center justify-center gap-4 text-3xl">
				<PrismicImage field={navbar.data.og_image} class="h-20 w-20" />
				<span class="">{navbar.data.title}</span>
			</a>
		...
		>
			...
			<ul class="grid justify-items-end gap-8">
				{#each navbar.data.navitem as item (item.label)}
					<li>
						{#if item.cta_button}
							<ButtonLink
								field={item.link}
